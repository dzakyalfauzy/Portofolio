...
export default function Dashboard() {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            let response;
            if (modalMode === "create") {
                response = await api.post(`/admin/${activeTab}`, payload, {
                    headers: { "Content-Type": "multipart/form-data" }
                });
            } else {
                // Laravel PUT request helper workaround for FormData uploads
                payload.append("_method", "PUT");
                response = await api.post(`/admin/${activeTab}/${selectedItem.id}`, payload, {
                    headers: { "Content-Type": "multipart/form-data" }
                });
            }
            ...
            const msg = err.response?.data?.message || "Failed to save record. Validate fields.";
            setFormError(msg);
        } finally {
